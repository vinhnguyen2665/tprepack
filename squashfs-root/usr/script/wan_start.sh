...
if [ "$TCSUPPORT_UNIQUEMAC" != "" ] ;then
	...
	if [ "$TCSUPPORT_IPV6_CEROUTER" != "" ] ;then
		ip6tables -I FORWARD -o nas$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
	else
	ip6tables -A FORWARD -o nas$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
	fi
...
if [ "$TCSUPPORT_UNIQUEMAC" != "" ] ;then
	...
	if [ "$TCSUPPORT_MULTISERVICE_ON_WAN" != "" ] && [ "$TCSUPPORT_WAN_PTM" != "" -o "$TCSUPPORT_WAN_ETHER" != "" ] && [ "$isPTMETHER" = "1" ]; then
		if [ "$LASTmss" != "0" ] && [ "$LASTmss" != "" ] ; then
			...
			ip6tables -D FORWARD -o ppp"$org_i""$serv_num" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $LASTmss
		fi
	
		if [ "$MSS" = "0" ] || [ "$MSS" = "" ] ; then
			iptables -A FORWARD -o ppp"$org_i""$serv_num" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
			ip6tables -A FORWARD -o ppp"$org_i""$serv_num" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
		else
			iptables -A FORWARD -o ppp"$org_i""$serv_num" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
			ip6tables -A FORWARD -o ppp"$org_i""$serv_num" -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
		fi
	else
		if [ "$LASTmss" != "0" ] && [ "$LASTmss" != "" ] ; then
			iptables -D FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $LASTmss
			ip6tables -D FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $LASTmss
		fi
		...
		if [ "$MSS" = "0" ] || [ "$MSS" = "" ] ; then
			...
			iptables -A FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
				ip6tables -A FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
			fi
		else
			if [ "$TCSUPPORT_FON_V2" != "" ] ; then
			...
			iptables -A FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
				ip6tables -A FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
		fi
	fi	
	fi	
	if [ "$IPVERSION" != "IPv6" ] ; then
...
if [ $i = "0" ] ;then
## BDCOM set ipOption, isp=1,nas0 will never up,so addif nas0 
	echo "add nas0 for BDCOM issue..."
	brctl addif br0 nas0
	ebtables -t filter -D INPUT -i nas0 -p IPv4 --ip-proto 17 --ip-dport 67 -j DROP
	ebtables -t filter -D INPUT -i nas0 -p IPv6 --ip6-proto 17 --ip6-dport 547 -j DROP
	ebtables -t filter -A INPUT -i nas0 -p IPv4 --ip-proto 17 --ip-dport 67 -j DROP
	ebtables -t filter -A INPUT -i nas0 -p IPv6 --ip6-proto 17 --ip6-dport 547 -j DROP
fi
#jrchen
#	if [ "$IPVERSION" = "IPv4/IPv6" ] ; then
#		sleep 1
